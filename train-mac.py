...
parser.add_argument('--bert_model_dir', default='hfl/chinese-macbert-base', help="Directory containing the BERT model in PyTorch")
...
def train(model, data_iterator, optimizer, scheduler, params):
    ...
    for i in t:
        ...
        #print("tags_batch size: ", batch_tags.size())

        # compute model output and loss
        #loss = model(batch_data, token_type_ids=None, attention_mask=batch_masks, labels=batch_tags)
        outputs = model(batch_data, token_type_ids=None, attention_mask=batch_masks, labels = batch_tags)
        # print("output size: ", outputs.size())
        loss = outputs[0]

        #loss = F.cross_entropy(outputs, batch_tags)
...
if __name__ == '__main__':
    ...
    model = model.BertSoftmaxForNer.from_pretrained(args.bert_model_dir, num_labels=len(params.tag2idx))
